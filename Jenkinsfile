def label = "shopagent"
  ...
  """
    ...
    node (label) {
        ...
        stage ('Checkout SCM'){
            git credentialsId: 'git', url: 'https://github.com/Kumar-arj/micro-service-front-end-deployment.git', branch:  "${env}"
        }
        ...
        stage ('Helm Chart') {
            container('build') {
                withCredentials([usernamePassword(credentialsId: 'nexuslogin', usernameVariable: 'username', passwordVariable: 'password')]) {
                    ...
                    sh "/usr/local/bin/helm upgrade  --install --force micro-services-front  --namespace ${env} -f values.yaml sock-shop-helm-local/micro-services-front"
